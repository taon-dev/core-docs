    ...
    <taon-admin-mode-configuration>
      @if (itemsLoaded()) {
        ...
        <footer
          ...
          {{ t.gettext('Copyright') }} <strong>core-docs</strong>
          {{ year }}
        </footer>
      }
    </taon-admin-mode-configuration>
...
export class CoreDocsApp implements OnInit {
  t = t.for(this);
  ...
  navItems =
    CoreDocsClientRoutes.length <= 1
      ? []
      : CoreDocsClientRoutes.filter(r => r.path !== undefined).map(r => ({
          path: r.path === '' ? '/' : `/${r.path}`,
...
//#region  core-docs routes
//#region @browser
export const CoreDocsServerRoutes: ServerRoute[] = [
  {
    path: '**',
    renderMode: RenderMode.Prerender,
  },
];
export const CoreDocsClientRoutes: Routes = [
  {
    path: '',
    pathMatch: 'full',
    redirectTo: () => {
      if (CoreDocsClientRoutes.length === 1) {
        return '';
      }
      return CoreDocsClientRoutes.find(r => r.path !== '')!.path!;
    },
  },
  // PUT ALL ROUTES HERE
...
//#region  core-docs app configs
//#region @browser
export const CoreDocsAppConfig: ApplicationConfig = {
  providers: [
    provideZonelessChangeDetection(),
    {
      provide: TAON_CONTEXT,
      useFactory: () => CoreDocsContext,
    },
    providePrimeNG({
      theme: {
        preset: Aura,
    ...
    }),
    {
      ...
      multi: true,
      useFactory: () => CoreDocsStartFunction,
    },
    provideBrowserGlobalErrorListeners(),
    // remove withHashLocation() to use SSR
    provideRouter(
      CoreDocsClientRoutes,
      withHashLocation(),
      withComponentInputBinding(),
    ),
...
export const CoreDocsServerConfig: ApplicationConfig = {
  providers: [provideServerRendering(withRoutes(CoreDocsServerRoutes))],
};

export const CoreDocsConfig = mergeApplicationConfig(
  CoreDocsAppConfig,
  CoreDocsServerConfig,
);
//#endregion
//#endregion

//#region  core-docs entity
...
//#region  core-docs context
var CoreDocsContext = Taon.createContext(() => ({
  ...HOST_CONFIG['CoreDocsContext'],
...
//#region  core-docs start function
export const CoreDocsStartFunction = async (
  ...
  await CoreDocsContext.initialize(startParams);
...
export default CoreDocsStartFunction;
//#endregion
